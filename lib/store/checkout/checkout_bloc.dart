...
//              successList.fold(true, (prev, success) => prev && success);
          ...
          if (success) {
            var res = await ZarinPalClient().getPaymentURL(
                ZPPaymentRequest(Price(paymentState.toPayAmount)));
      ...
      if (loginState is IsLoggedIn) {
        if (currentState is LaunchGateway) {
          ...
          var res = await ZarinPalClient().verifyPayment(
              ZPVerifyRequest(gatewayState.authority, gatewayState.amount));

          if (res is ZPVerifySuccess) {
