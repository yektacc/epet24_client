...
import 'package:store/data_layer/error/error_reporting_repository.dart';
...
import 'package:store/test_app.dart';
...
Future main() async {
  ...
  final ConstantsRepository _constantsRepo = ConstantsRepository(net);
  final ConstantsBloc _constantsBloc = ConstantsBloc(_constantsRepo);
  final LandingBloc _landingBloc =
      LandingBloc(_provinceBloc, _structureBloc, _constantsBloc);
...
//  final CouponBloc _couponBloc = CouponBloc(_couponRepo);
  ...
  final CheckoutBloc _checkoutBloc = CheckoutBloc(_deliveryPriceRepo, _cartRepo,
      _loginStatusBloc, _centersRepo, _detailRepo, _orderRepo, _couponRepo);

  final ErrorReportRepository _errorRepo = ErrorReportRepository(net);
  final GRepo _gRepo = GRepo(net);

  // get it classes to get gettable
  getIt.registerSingleton<Net>(net);
  getIt.registerSingleton<InboxManager>(_inboxManager);

  getIt.registerSingleton<LoginStatusBloc>(_loginStatusBloc);
  getIt.registerSingleton<GRepo>(_gRepo);
  getIt.registerSingleton<ConstantsBloc>(_constantsBloc);

  runZonedGuarded<Future<void>>(() async {
    _landingBloc.state.listen((event) {
      if (event is LoadingSuccessful) {
        // runApp(TestApp());
        runApp(App([
...
//    Provider<ShopManagementBloc>.value(value: _shopManagementBloc),
    ...
    /*   if (_landingBloc.state is LoadingSuccessful) {
    }*/


  }, (Object error, StackTrace stackTrace) {
    _errorRepo.reportError(error, stackTrace);
  });
}
